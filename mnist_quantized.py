...
def linear_u8_s8(x_u8, w_i8, trunc_bits=0):
...
def run_inference(x_test, y_test, weights, trunc_bits, hidden_scale):
...
def main():
    ...
    parser.add_argument("--out", default="mnist_quantized_results.csv")
    ...
    for trunc_bits in range(8):
        accuracy = run_inference(x_test, y_test, weights, trunc_bits, hidden_scale)
        rows.append({"trunc_bits": trunc_bits, "accuracy": accuracy})
        print(f"T={trunc_bits}: accuracy={accuracy:.4f}")

    with open(args.out, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["trunc_bits", "accuracy"])
        writer.writeheader()
